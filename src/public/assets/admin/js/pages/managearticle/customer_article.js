...
Use : Validate customer article list form and submit data to db
*/
$(document).ready(function () {
    $('#add_customer_form').validate({
        ...
        submitHandler: function (form) {
            ...
            console.log(data);
            $.ajax({
                url: BASE_URL + '/save_customer_data',
                type: 'POST',
                ...
                success: function (result) {
                    if (result.status_code == 200) {
                        ...
                        setTimeout(function () {
                            window.location.href = BASE_URL + '/cms/news_ideas/customer';
                        }, 1000);
...
$(document).ready(function () {
    $('#edit_customer_form').validate({
        ...
        submitHandler: function (form) {
            ...
            console.log(data);
            $.ajax({
                url: BASE_URL + '/update_customer_data',
                type: 'POST',
                headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
                data: data, id: id,
                contentType: false,
...
$(document).on('click', '.status_onoffswitchcustomer', function () {
    let id = $(this).data('id');
    let is_active = 0;
...
Created by : Chandan Yadav
Created at : 03 November 2023
Use : Delete customer article list data.
*/

$(document).on('click', '#delete_btn', function () {
    let bride_id = $(this).data('id');
    $('#delete_customer_modal').modal('show');
    $('#customer_id_delete').val(bride_id);
});


$(document).on('click', '#delete_customer_form_btn', function (event) {
    let id = $('#customer_id_delete').val();
    $('#delete_customer_modal').modal('hide');
    event.preventDefault();
    $.ajax({
        url: BASE_URL + "/delete_customer_data",
        type: 'DELETE',
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        data: {
            id: id, _token: CSRF,
...
*/
$(document).ready(function () {
    $('#customer_article_list').DataTable({
        searching: true
    });
});
